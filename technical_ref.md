...
│   │   ├── BidRequest.java             # 出价请求(roomId, amount, itemType)
...
│   └── stores/
│       └── game.js                     # Pinia: WS连接+游戏状态(含itemResults)
...
```
WAITING ──startGame()──→ SKILL_PHASE ──timer──→ BIDDING ──timer──→ GRACE_PERIOD ──timer──→ EVALUATING
  ↑                                                                                              │
  │                                                                                    ┌─────────┼─────────┐
  │                                                                                    ▼         ▼         ▼
...
**注意：**
- SKILL_PHASE 状态仍在，但前端只显示小技能栏（非大块UI）
- 技能由玩家手动触发 REST 端点，不再自动 `executeAllSkills()`
...
| 方法 | 作用 |
|------|------|
| `createRoom(hostId)` | 创建房间(UUID)，hostId 设为房主 |
...
| `startGame(roomId, requesterId)` | 开始游戏(生成仓库 → initPlayerStates → enterSkillPhase) |
| `initPlayerStates(room)` | 从 pendingPurchases 扣金币、道具写入 Redis，清空 pendingPurchases |
| `enterSkillPhase(roomId)` | 进入技能阶段(仅状态变更+广播，不自动执行技能) |
...
| `useItem(roomId, pid, itemType)` | 独立使用道具（独立于出价，通过WS调用）从背包移除→存pendingBuff/效果→返回结果 |
...
| `GET /api/room/{roomId}` | getRoom | 查房间(warehouse/playerIds/playerCharacters) |
...
| `SKILL_PHASE_START` | 技能阶段开始 | `{round}` | state=SKILL_PHASE, 重置 playerBids |
| `ROUND_START` | 出价阶段开始 | `{round, deadlineTs}` | state=BIDDING, 重置 playerBids |
| `BID_PLACED` | 有人出价 | `{playerId, count, total}` (顶层) | `playerBids[playerId]=true` |
...
| `/user/queue/reconnect` | `{action, state, round, deadlineTs, hasBidThisRound, bidderIds, coins, items}` | 重连快照(含背包恢复) |
...
1. 大厅阶段
   ...
   POST /api/room/{roomId}/start → startGame()
      ...
      └─ enterSkillPhase(roomId)            → 开始第1轮

2. 技能阶段 (SKILL_PHASE)
   SKILL_PHASE_START 广播 → 技能栏
   POST /api/room/{roomId}/skill?category=X → useSkill() (手动, SETNX防重)
   timer到期 → enterBidding()
...
6. 判定 (EVALUATING)
   triggerEvaluation()
      ├─ ALL_BIDS 广播
      ├─ BidEvaluator.evaluate() → 速胜/平局/继续
      ├─ 保险退款 + 截胡减半
...
| `game:player:state:{roomId}:{pid}` | Hash | `{character, coins, items[], hasBidThisRound, skillUsed, pendingBuff}` | 玩家状态(含待处理增益) | 2h |
...
### 生成逻辑
`RngManager.generate()`：
1. 随机决定仓库大小（`warehouseSizeMin` ~ `warehouseSizeMax`）
2. 按品质权重随机抽取物品模板（从 DB 的 `item_template` 表）
3. 30%概率追加黑匣子
4. `assignGridPositions()` 分配网格位置（10列紧凑排列）
...
### 拍卖轮次配置
`GameConfig.totalRounds`（默认 10）替代 `speedWinRounds + finalRounds`。`enterSkillPhase()` 优先使用 `totalRounds`，为 null 时回退旧逻辑。
...
| submitBid (WS) / reconnect (WS) | 5次/60s |
